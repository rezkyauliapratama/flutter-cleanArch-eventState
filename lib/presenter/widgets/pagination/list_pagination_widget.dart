...
import 'package:movies_db_bloc/presenter/widgets/pagination/pagination_helper_widget.dart';
...
typedef Widget PaginationBuilder<T>(PaginationState<T> state);
...
/// memory
///
/// You can build your own Pagination widgets by extending this class and
/// returning your builder in the [builder] function which provides you with the
/// Pagination state. Look [PaginationListView] and [PaginationGridView] for examples.
///
/// See also:
///
///  * [PaginationGridView], a [Pagination] implementation of [GridView](https://docs.flutter.io/flutter/widgets/GridView-class.html)
///  * [PaginationListView], a [Pagination] implementation of [ListView](https://docs.flutter.io/flutter/widgets/ListView-class.html)
abstract class Pagination<T> extends StatefulWidget {
  ...
  /// The actual builder that builds the Pagination widget. It is called and
  /// provided the PaginationState. This function is important only for classes
  /// extending Pagination. See [PaginationListView] and [PaginationGridView] for
  /// examples.
  ...
  /// listen to the data that Pagination fetches. Review the documentation of
  /// [PaginationLoadController] for more details
  final PaginationLoadController<T> pageLoadController;
...
class PaginationState<T> extends State<Pagination<T>> {
  PaginationLoadController<T> _controller;

  PaginationLoadController<T> get _effectiveController =>
  ...
  void initState() {
    ...
    if (widget.pageLoadController == null) {
      this._controller = PaginationLoadController<T>(
...
/// The controller responsible for managing page loading in Pagination
///
/// You don't have to provide a controller yourself when creating a Pagination
...
///
/// ```dart
/// final _pageLoadController = PaginationLoadController(
///   pageSize: 6,
...
/// listen to the state of Pagination and act accordingly.
...
/// }
/// ```
class PaginationLoadController<T> extends ChangeNotifier {
  List<T> _loadedItems;
  List _appendedItems;
  ...
  /// Creates a PaginationLoadController.
  ///
  /// You must provide both the [pageFuture] and the [pageSize]
  PaginationLoadController({@required this.pageFuture, @required this.pageSize});
  ...
  Future<void> fetchNewPage() async {
    if (!this._isFetching) {
      ...
      if (length > 0 && length < this.pageSize) {
        ...
        // similar to normal pages. This is useful especially with 3,
...
class PaginationListView<T> extends Pagination<T> {
  /// Creates a Pagination ListView.
  ///
  ...
  /// or those inherited from [Pagination]
  PaginationListView(
      {Key key,
        ...
        PaginationLoadController<T> pageLoadController,
        double itemExtent,
      ...
      errorBuilder: errorBuilder,
      noItemsFoundBuilder: noItemsFoundBuilder,
      builder: (PaginationState<T> state) {
...
class PaginationGridView<T> extends Pagination<T> {
  /// Creates a Pagination GridView with a crossAxisCount.
  ///
  ...
  /// Creates a Pagination GridView with a maxCrossAxisExtent.
  ///
  /// All the properties are either those documented for normal [GridViews](https://docs.flutter.io/flutter/widgets/GridView-class.html)
  /// or those inherited from [Pagination]
...
class PaginationSliverList<T> extends Pagination<T> {
  /// Creates a Pagination SliverList.
  ///
...
class PaginationSliverGrid<T> extends Pagination<T> {
  /// Creates a Pagination SliverGrid with a crossAxisCount.
  ///
  ...
  /// Creates a Pagination SliverGrid with a maxCrossAxisExtent.
  ///
  /// All the properties are either those documented for normal [SliverGrid](https://docs.flutter.io/flutter/widgets/SliverGrid-class.html)
  /// or those inherited from [Pagination]
  PaginationSliverGrid.extent(
      {Key key,
        bool addSemanticIndexes = true,
        bool addAutomaticKeepAlives: true,
        bool addRepaintBoundaries: true,
